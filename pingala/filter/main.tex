...
\end{figure}
\newline
From \ref{fig:xan}: we see that
\begin{align}
	x(n) = a_{n + 1}	\quad	n \geq 1
\intertext{Apply $\mathcal{Z}^+$ transform on both sides}
=1 + \sum_{n = 1}^{\infty}a_{n+1}z^{-n} \\
	=\sum_{n=0}^{\infty} \dfrac{\alpha^{n+1} - \beta^{n+1}}{(\alpha - \beta)}z^{-n}\\
