...
def fast_fourier(x, n, X ,s = 1):
    if n == 1:
        X[0] = x[0]
    else:
        X[int(n/2) - 1::2] = fast_fourier(x, int(n/2), X,2 * s)
        X[int(n/2)::2] = fast_fourier(x[s:], int(n/2), X,2 * s)
        for k in range(int(n/2) - 1):
            p = X[k]
            q = np.vdot(np.exp(-1j * 2 * np.pi * k / n), X[k+int(n/2)])
            X[k] = p + q 
            X[k+int(n/2)] = p - q
    return X
...
X = [0]*N
Xo = []
Xo.append( fast_fourier(xn,N,X,1))
X = np.real(Xo)/N
