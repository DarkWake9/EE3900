...
\begin{align}
	A = -10\log_{10}(G^2(f)) = -20\log_{10}(H_n(f))
\end{align}
At $f = f0$, A = -3 dB
\item Design a Chebyschev filter for $H(f)$.
\solution\\
For an $n^{th}$ order Chebyschev filter:\\
The Gain response $G(f)$ is given by\\
\begin{align}
	G(f) = H_n(f) = \frac{1}{\sqrt{1 + \epsilon^2 T^2_n\frac{f}{f_0}}}
\end{align}
Where :
\begin{enumerate}

\item $f_0$ is the cutoff frequency.\\
\item $T_n$ is a Chebyshev polynomial of the $n^{th}$ order.
\begin{align}
	T_n = \begin{cases}
		\cos \brak{n \cos^{-1} x} & \abs{x} < 1\\
		\cosh \brak{n \cosh^{-1} x} & \abs{x} \geq 1
	\end{cases}
\end{align}
